class ScholarshipsController < ApplicationController
  ...
  def index
    ...
    if params[:applicant]
      session[:applicant] = params[:applicant]
      @selected_applicant = params[:applicant]
    else
      @selected_applicant = 'regular'
    end
    if @selected_applicant = 'regular'
      if sort?(params)
        @sort = params[:sort]
        session[:sort] = @sort
        @scholarships = Scholarship.order(@sort).reverse_order.all
      else
